...
typedef struct {
    va_list ap;
    const char *fmt;        /* printf msg */
    const char *mod;        /* different module used */
    const char *file;       /* printf file name */
    const char *func;       /* printf function name */
    struct tm *time;        /* printf current time */
    void *udata;            /* output stream */
    int line;               /* printf line number */
    int level;              /* log level */
} log_event;
...
/*
 eg:
    ...
    // use (alock is a global variable) 
...
/* FILE_NAME */
#define GET_FILENAME_FROM_PATH(path) strrchr((const char*)path, '\\') ? strrchr((const char*)path, '\\') + 1 : \
                                     strrchr((const char*)path, '/') ? strrchr((const char*)path, '/') + 1 : (const char*)path

#define BASENAME_FILE() strrchr((const char*)__FILE__, '\\') ? strrchr((const char*)__FILE__, '\\') + 1 : \
...
/*loglevel>=4*/
#if loglevel>=LOG_DEBUG
#define log_debug(...) log_log(LOG_DEBUG, BASENAME_FILE(), __LINE__, __func__, __VA_ARGS__)
#else
#define log_debug(...) 
#endif 
...
/*loglevel>=1*/
#if loglevel>=LOG_ERROR
#define log_error(...) log_log(LOG_ERROR, BASENAME_FILE(), __LINE__, __func__, __VA_ARGS__)
#else
#define log_error(...) 
#endif 
...
/*loglevel>=0*/
#if loglevel>=LOG_FATAL
#define log_fatal(...) log_log(LOG_FATAL, BASENAME_FILE(), __LINE__, __func__, __VA_ARGS__)
#else
#define log_fatal(...)
#endif 
#endif
