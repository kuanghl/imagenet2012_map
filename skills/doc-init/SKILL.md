...
| 角色 | 说明 |
|------|------|
| **事实源** | **当前应用本 skill 的目标项目代码仓库**（工作区 / 用户指定的工程路径）。所有章节内容须从该仓库的源码、配置与目录结构 **推导与核对**，**完整分析所有代码文件**（`.c`、`.cpp`、`.h`、`.py`、`.java`、`.go`、`.ts`、`.js` 、`.sh`等），不得把「skills 集合仓库」里的示例当成业务真相（除非正在文档化的就是 skills 本身）。 |
...
**Quick Start：** 锁定 **目标项目根目录** → 确认文档语言 → **完整扫描所有源码文件**（含头文件、实现文件、配置文件）→ 分析该仓库目录与技术栈 → 提取函数接口、结构体/类定义 → **按 doxygen 格式生成 API 与类型文档** → 按 [doc-init.md](examples/doc-init.md) 在目标项目中生成 `docs/src/` 与 `book.toml` / `SUMMARY.md` → 按需嵌入扩展图示 → 列出产出树并提示用 **doc-update** 维护。
...
| 维度 | 要求 |
|------|------|
| **完整性** | 覆盖仓库真实入口、构建/运行方式、**完整分析所有源码文件**（头文件、实现文件）、对外接口、核心数据结构与错误排查入口；为每个代码文件输出 **doxygen 格式的函数接口与结构体/类说明文档**；无「待补充」占位的关键章节（若信息缺失，明确标注 **待确认项** 并列出需用户提供的输入）。 |
...
| `api.md` | 对外接口表格或列表：**路径或符号名、方法、参数、响应/返回值、鉴权、错误语义**；必要时链接到 `structures.md`；**按 doxygen 格式输出函数接口文档**（含 `@brief`、`@param`、`@return`、`@note` 等标签）。 |
| `structures.md` | 核心类型/表结构字段说明；与源码定义同步的字段名与必选/可选；**按 doxygen 格式输出结构体/类文档**（含 `@struct`、`@class`、`@field`、`@brief` 等标签）。 |
...
| **mdBook + Mermaid 为基准** | 架构/流程/数据模型优先用 Mermaid；配置对齐 **[assets/book.toml](assets/book.toml)** + **`mermaid-init.js` / `mermaid.min.js`**（详见 [examples/doc-init.md](examples/doc-init.md)）。 |
| **图表与代码一致** | `architecture.md`、`principles.md`、`api.md` 等与 **目标项目** 源码、接口真相一致。 |
| **完整代码文件分析** | 不跳过任何源码文件；遍历所有 `.c`、`.cpp`、`.h`、`.py`、`.java`、`.go`、`.ts`、`.js` 、`.sh`等文件提取接口与类型定义。 |
| **doxygen 格式输出** | 函数接口文档使用 `@brief`、`@param`、`@return`、`@note`；结构体/类使用 `@struct`、`@class`、`@field`、`@brief`；保持与 doxygen 标准兼容。 |
...
| **mdBook + Mermaid 参考（复制到目标项目 `docs/`）** | [book.toml](assets/book.toml)、[mermaid-init.js](assets/mermaid-init.js)、[mermaid.min.js](assets/mermaid.min.js) |
...
1. **语言：** 确认文档语言（默认中文）。
2. **代码文件扫描：** **完整遍历所有源码文件**（`.c`、`.cpp`、`.h`、`.py`、`.java`、`.go`、`.ts`、`.js` 、`.sh`等），提取函数接口、结构体/类定义。
3. **分析：** 目录树、入口与清单文件、依赖、对外接口与核心类型。
4. **生成：** 在 `docs/src/` 下建立 README、architecture、api、modules、submodules 等（详见示例）；**按 doxygen 格式生成函数接口与结构体/类文档**。
5. **mdBook：** 在目标项目 `docs/` 对齐 **[book.toml](assets/book.toml)**，并放入 **[mermaid-init.js](assets/mermaid-init.js)**、**[mermaid.min.js](assets/mermaid.min.js)**；安装 `mdbook` 与 `mdbook-mermaid`（详见 [examples/doc-init.md](examples/doc-init.md) 第 5 节）。
6. **可选增强：** 按领域从该示例「Skill 融合」表选用 [architecture](../architecture/SKILL.md)、[uml](../uml/SKILL.md)、[cloud](../cloud/SKILL.md) 等 skill 生成附加图示或卡片。
7. **自检：** 遍历 `SUMMARY.md` 链接；核对图表与源码路径；检查术语与 `glossary.md`；运行 `mdbook build`（若环境可用）确认 Mermaid 与预处理无报错。
8. **交付：** 打印目录树与 **验收摘要**（已生成页面列表、已知待确认项、快照路径）；提示 `mdbook serve --open`；说明后续使用 **`/doc-update`** 增量维护，**`/code-review`** 结论可经 doc-update 写入 Wiki。
...
| Mermaid 在 CI 不渲染 | 在 CI 安装 `mdbook-mermaid`，并确保 `book.toml` 中 `[preprocessor.mermaid]` 与 [additional-js](assets/book.toml) 指向的脚本已随仓库提交或由构建步骤提供。 |
